...
namespace WebAutoCodeOnline.MySqlDAL
{
    public class ProjectTypeInfoDAL
    {
        ...
        public List<ProjectTypeInfo> GetAllProjectTypeList(bool containsDeleted)
        {
            ...
            {
                using (MySqlDataReader sqldr = MySqlHelper2.ExecuteDataReader(sqlcn, System.Data.CommandType.Text, selectAllProject))
                {
        ...
        public ProjectTypeInfo GetProjectTypeInfo(int proId, bool containsDeleted)
        {
            ...
            listParams.Add(new MySqlParameter("@ProjectTypeId", MySqlDbType.Int32) { Value = proId });
            using (MySqlConnection sqlcn = ConnectionFactory.AliDb)
            {
                using (MySqlDataReader sqldr = MySqlHelper2.ExecuteDataReader(sqlcn, System.Data.CommandType.Text, selectAllProject, listParams.ToArray()))
                {
                    if (sqldr.Read())
                    {
